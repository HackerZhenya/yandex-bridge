...
**Обратная сторона.** HomeKit кэширует состав сервисов аксессуара и плохо
переваривает «тот же `aid`, но теперь другой набор» — приложение Дом может
показывать старые элементы управления бесконечно. Поэтому вместе с `aid`
хранится хеш формы аксессуара, и при её реальном изменении (чайник стал
термостатом, лампа обзавелась цветом) устройство получает **новый** `aid`:
старое исчезает, новое появляется — такой переход HomeKit понимает. Затрагивается
только изменившееся устройство, остальные номера не двигаются. В логе это
всегда видно предупреждением.
...
видно в `/devices` и в логах при старте, вместе с `instance` каждого умения.

**Кнопки.** Zigbee-кнопки Яндекс типизирует как `devices.types.switch`, но
`on_off` у них нет — только `devices.properties.event` с instance `button`.
Такое устройство теперь **не** экспортируется: раньше получался тумблер, который
ничего не переключал. В HomeKit для кнопок есть `StatelessProgrammableSwitch`, но
у пользовательского API Яндекса нет push, и нажатие приезжало бы с задержкой до
интервала опроса.

**Недокументированные умения.** `devices.capabilities.identify` и
`devices.capabilities.zigbee_node` отсутствуют в официальном списке типов —
Яндекс добавляет их сам для Zigbee-устройств. Формат действия неизвестен,
поэтому мост их не трогает, но показывает в `unmapped` вместе с `instance`.
