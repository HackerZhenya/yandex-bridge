...
настройках плитки. Поэтому розетка с термометром остаётся одной розеткой, а
чайник — одной плиткой с регулятором температуры и переключателями режимов.

Кондиционеры (`thermostat.ac`) в Thermostat не превращаются: им нужен режим
охлаждения, это отдельная задача.
...
### Что за устройства нашлись

Мост печатает полную инвентаризацию при старте и при каждом изменении состава
устройств: id, имя, тип, комнату, во что он это превратил и — главное — **какие
умения не поддержаны**. То же самое в JSON:

```bash
curl -s http://raspberrypi.local:8080/devices | jq
```

```json
{
  "device_id": "78c6e9de-ab96-48e1-84d6-099ea0474f72",
  "name": "Чайник",
  "type": "devices.types.cooking.kettle",
  "room": "Кухня",
  "homekit_kind": "thermostat",
  "mapped": ["on_off → On", "range:temperature → TargetTemperature",
             "float:temperature → CurrentTemperature", "toggle:keep_warm → Switch"],
  "unmapped": ["devices.capabilities.mode", "float:water_level"]
}
```

Отсюда берутся `device_id` для конфига, и сразу видно, что теряется.
...
devices:
  ...
  # Чайник определяется автоматически и становится термостатом. Вернуть
  # прежний вид — выключатель плюс отдельный датчик температуры:
  "kettle-id-yyy":
    type: switch
...
Две настройки на случай, если что-то ведёт себя не так:

- **`coalesce_delay`** (60 мс) — окно склейки. HomeKit шлёт Hue и Saturation
  одним запросом, но `hap` отдаёт их по одной характеристике за раз, поэтому без
  склейки одно изменение цвета превращается в два запроса к Яндексу, и первый
  уходит со старым компонентом — лампа успевает мигнуть неправильным оттенком.
- **`settle_window`** (10 с) — сколько записанное из HomeKit значение защищено от
  перезаписи ещё не догнавшим опросом. Без этого ползунок в палитре отскакивает
  назад под пальцем. Увеличьте, если ваши устройства медленно рапортуют;
  уменьшите, если изменение из приложения Яндекса появляется слишком долго.
...
Рядом лежит переключатель повторной авторизации. Он стоит во **включённом**
положении — это спокойное состояние; **выключите** его, чтобы заново запустить
device flow, не заходя на Pi по SSH, после чего он сам вернётся обратно. Код
появится в логах и в `/healthz`.

Весь аксессуар отключается через `health.enabled: false`, отдельно кнопка — через
`health.reauth_button: false`.
...
internal/
  ...
  bridge/
    ...
    accessory.go        сборка аксессуаров, склейка записей, подавление эха
    sync.go             опрос, диффинг, подтверждения
    inventory.go        отчёт «что нашли и что не смогли смапить»
    health.go           аксессуар Bridge Health
  config/               env + YAML
  logging/              slog + мост из hap/log
  status/               /healthz, /devices
...
Ещё три — регрессии на баги, найденные в живой эксплуатации:

```bash
docker run --rm -v "$PWD":/src -w /src golang:1.26-alpine \
  go test ./internal/bridge/ -run 'Colour|StaleState|ConfirmationsDoNotBlock' -v
```

- `TestColourChangeIsASingleAction` — одно изменение цвета = один запрос к
  Яндексу с правильными Hue и Saturation, а не два с промежуточным оттенком.
- `TestStaleStateDoesNotOverwriteRecentWrite` — отставший опрос не возвращает
  ползунок назад.
- `TestConfirmationsDoNotBlockPolling` — очередь подтверждений не останавливает
  обычный опрос.
...
Осознанно вне текущей версии: кондиционеры (`thermostat.ac`, нужен режим
охлаждения), увлажнители и очистители, шторы, замки и клапаны (`openable.*`),
медиаустройства, камеры, пылесосы, `devices.capabilities.mode` (режимы вроде
`tea_mode`), сценарии и группы. Модель расширяемая — добавление типа это новая
...
Switch; всё остальное пропускается, а не угадывается. Что именно пропущено —
видно в `/devices` и в логах при старте.

**Известное ограничение.** У чайника целевая температура 40–100 °C выходит за
диапазон, который спецификация HAP отводит `TargetTemperature` (10–38 °C). Мост
расширяет границы через метаданные характеристики — так делают плагины Homebridge
для водонагревателей, и приложение Дом берёт границы из того, что объявил
аксессуар. Это единственное место, где поведение iOS не проверено заранее: если
регулятор упрётся в 38 °C, обходной путь — отдельные переключатели-пресеты.
